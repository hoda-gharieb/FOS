...
struct UserProgramInfo userPrograms[] = {
		...
		{ "tB1", "Tests the shared variables", PTR_START_OF(tst_sharing_5_slaveB1)},
		{ "tB2", "Tests the shared variables", PTR_START_OF(tst_sharing_5_slaveB2)},
		...
		{ "tsre2", "Tests the realloc", PTR_START_OF(tst_realloc_2)},
		{ "tsre1", "tests the realloc", PTR_START_OF(tst_realloc_3)},
		{ "fr1", "Tests the realloc", PTR_START_OF(tst_freeRAM_1)},
				{ "fr2", "tests the realloc", PTR_START_OF(tst_freeRAM_2)},
};
	...
	uint32 cd = 1024*4*1024;
		...
		for( i=0 ; i<1024 ; i++ )
		{
			if(  (e->env_pgdir[ i ] & PERM_PRESENT) != 0 && ((i*cd) < USER_TOP))
			{
				...
				get_page_table( e->env_pgdir, (void*)(i*cd), &ptr_page_table);
				int j;
				for( j=0 ; j<1024 ; j++ )
				{

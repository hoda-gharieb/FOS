...
void initialize_kernel_VM() {
	...
	boot_map_range(ptr_page_directory, KERNEL_STACK_TOP - KERNEL_STACK_SIZE,
			KERNEL_STACK_SIZE, K_PHYSICAL_ADDRESS(ptr_stack_bottom),
			PERM_WRITEABLE);
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here: 
	boot_map_range(ptr_page_directory, KERNEL_BASE, 0xFFFFFFFF - KERNEL_BASE,
			0, PERM_WRITEABLE);
	...
	boot_map_range(ptr_page_directory, READ_ONLY_FRAMES_INFO, array_size,
			K_PHYSICAL_ADDRESS(frames_info), PERM_USER);
	...
	boot_map_range(ptr_page_directory, UENVS, envs_size,
			K_PHYSICAL_ADDRESS(envs), PERM_USER);
...
void boot_map_range(uint32 *ptr_page_directory, uint32 virtual_address,
	...
	for (i = 0; i < size; i += PAGE_SIZE) {
		...
		ptr_page_table[index_page_table]
				= CONSTRUCT_ENTRY(physical_address, perm | PERM_PRESENT);
...
uint32* boot_get_page_table(uint32 *ptr_page_directory, uint32 virtual_address,
	...
	if (phys_page_table == 0) {
		if (create) {
			...
			ptr_page_directory[index_page_directory]
					= CONSTRUCT_ENTRY(phys_page_table, PERM_PRESENT | PERM_WRITEABLE);
...
int allocate_frame(struct Frame_Info **ptr_frame_info) {
	*ptr_frame_info = LIST_FIRST(&free_frame_list);
	if (*ptr_frame_info == NULL) {
		...
		for (i = 0; i < NENV && *ptr_frame_info == NULL; i++) {
			struct Env* e;
...
int get_page_table(uint32 *ptr_page_directory, const void *virtual_address,
	...
	*ptr_page_table = K_VIRTUAL_ADDRESS(EXTRACT_ADDRESS(page_directory_entry));
	...
	} else if (page_directory_entry != 0) //the table exists but not in main mem, so it must be in sec mem
	{
		...
		*ptr_page_table
				= K_VIRTUAL_ADDRESS(EXTRACT_ADDRESS(page_directory_entry));
...
int create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address,
		uint32 **ptr_page_table) {
...
int map_frame(uint32 *ptr_page_directory, struct Frame_Info *ptr_frame_info,
	...
	uint32 *ptr_page_table;
	if (get_page_table(ptr_page_directory, virtual_address, &ptr_page_table)
			== TABLE_NOT_EXIST) {
		/*==========================================================================================
		 // OLD WRONG SOLUTION
		 //=====================
...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size) {
	...
	int y = ROUNDUP( size , 1024*4 ) / PAGE_SIZE;
	struct Frame_Info *ptr_frame_info;
	uint32 * dir = e->env_pgdir;
	int i = 0;
	for (i = 0; i < y; i++) {
		int r = allocate_frame(&ptr_frame_info);
		int m = map_frame(dir, ptr_frame_info, (void *) virtual_address,
...
uint32 calculate_required_frames(uint32* ptr_page_directory,
	...
	long i = 0;
	uint32 current_virtual_address =
			ROUNDDOWN(start_virtual_address, PAGE_SIZE*1024);

	for (; current_virtual_address < (start_virtual_address + size); current_virtual_address
			+= PAGE_SIZE * 1024) {
		uint32 *ptr_page_table;
...
inline void env_page_ws_invalidate(struct Env* e, uint32 virtual_address) {
	int i = 0;
	for (; i < PAGE_WS_MAX_SIZE; i++) {
		if (ROUNDDOWN(e->pageWorkingSet[i].virtual_address,PAGE_SIZE)
...
inline void env_page_ws_set_entry(struct Env* e, uint32 entry_index,
	...
	e->pageWorkingSet[entry_index].virtual_address
			= ROUNDDOWN(virtual_address,PAGE_SIZE);
...
void env_page_ws_print(struct Env *curenv) {
	...
	for (i = 0; i < PAGE_WS_MAX_SIZE; i++) {
		...
		cprintf("address @ %d = %x", i,
				curenv->pageWorkingSet[i].virtual_address);

		cprintf(", used= %d, modified= %d, buffered= %d, time stamp= %d",
...
void env_table_ws_print(struct Env *curenv) {
	...
	for (i = 0; i < TABLE_WS_MAX_SIZE; i++) {
		...
		cprintf("env address at %d = %x", i,
				curenv->tableWorkingSet[i].virtual_address);

		cprintf(", used bit = %d, time stamp = %d", pd_is_table_used(curenv,
				virtual_address), curenv->tableWorkingSet[i].time_stamp);
		if (i == curenv->table_last_WS_index) {
			cprintf(" <--");
		}
		cprintf("\n");
...
inline uint32 env_table_ws_get_size(struct Env *e) {
	...
	for (; i < TABLE_WS_MAX_SIZE; i++)
		if (e->tableWorkingSet[i].empty == 0)
			counter++;
	return counter;
}
...
inline void env_table_ws_invalidate(struct Env* e, uint32 virtual_address) {
	int i = 0;
	for (; i < TABLE_WS_MAX_SIZE; i++) {
		if (ROUNDDOWN(e->tableWorkingSet[i].virtual_address,PAGE_SIZE*1024)
				== ROUNDDOWN(virtual_address,PAGE_SIZE*1024)) {
...
inline void env_table_ws_set_entry(struct Env* e, uint32 entry_index,
	...
	assert(virtual_address >= 0 && virtual_address < USER_TOP);
	e->tableWorkingSet[entry_index].virtual_address
			= ROUNDDOWN(virtual_address,PAGE_SIZE*1024);
...
inline uint32 pd_is_table_used(struct Env* ptr_env, uint32 virtual_address) {
	return ((ptr_env->env_pgdir[PDX(virtual_address)] & PERM_USED) == PERM_USED ? 1
			: 0);
}
...
inline void pt_set_page_permissions(struct Env* ptr_env,
		uint32 virtual_address, uint32 permissions_to_set,
		uint32 permissions_to_clear) {
	uint32 * ptr_pgdir = ptr_env->env_pgdir;
	uint32* ptr_page_table;
	...
	} else if (page_directory_entry != 0) //the table exists but not in main mem, so it must be in sec mem
	{
		...
		int success = pf_read_env_table(ptr_env, virtual_address,
				(void*) ptr_temp_page);
...
inline void add_frame_to_storage(uint32* frames_storage,
	...
	ptr_page_table[PTX(va)]
			= CONSTRUCT_ENTRY(to_physical_address(ptr_frame_info), 0 | PERM_PRESENT);
}
...
inline struct Frame_Info* get_frame_from_storage(uint32* frames_storage,
	...
	uint32 va = index * PAGE_SIZE;
	ptr_frame_info
			= get_frame_info(frames_storage, (void*) va, &ptr_page_table);
